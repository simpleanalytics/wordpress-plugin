...
const INACTIVE_ADMIN_SCRIPT_SELECTOR = 'script[src="http://localhost:8888/wp-content/plugins/simpleanalytics/resources/js/inactive.js"]';
...
it('adds a script by default', function () {
    visit('http://localhost:8888')->assertPresent(DEFAULT_SCRIPT_SELECTOR);
});
...
it('adds a script with ignored pages', function () {
    ...
    visit('http://localhost:8888')->refresh()->assertSourceHas('data-ignore-pages="/vouchers"');
});

it('adds inactive script for selected user roles', function () {
    $admin = asAdmin()->navigate('http://localhost:8888/wp-admin/options-general.php?page=simpleanalytics&tab=ignore-rules')
    ...
    $admin->navigate('http://localhost:8888')
        ->assertPresent(DEFAULT_SCRIPT_SELECTOR);

    asAuthor()->navigate('http://localhost:8888')
        ->assertPresent(INACTIVE_ADMIN_SCRIPT_SELECTOR)
        ->assertSourceHas(INACTIVE_ADMIN_COMMENT);

    asEditor()->navigate('http://localhost:8888')
        ->assertPresent(INACTIVE_ADMIN_SCRIPT_SELECTOR)
        ->assertSourceHas(INACTIVE_ADMIN_COMMENT);
});

it('adds a script with collect do not track enabled', function () {
    asAdmin()->navigate('http://localhost:8888/wp-admin/options-general.php?page=simpleanalytics&tab=advanced')
        ->check('simpleanalytics_collect_dnt')
        ->click('Save Changes')
        ->assertChecked('simpleanalytics_collect_dnt');

    visit('http://localhost:8888')->assertSourceHas('data-collect-dnt="true"');
});
...
it('adds a script with hash mode enabled', function () {
    ...
    visit('http://localhost:8888')->assertSourceHas('data-mode="hash"');
});
...
it('adds a script with manually collect page views enabled', function () {
    ...
    visit('http://localhost:8888')->assertSourceHas('data-auto-collect="true"');
});
...
/*it('adds noscript tag when support no javascript mode is enabled', function () {
    ...
    visit('http://localhost:8888')->assertPresent(NOSCRIPT_SELECTOR);
});*/
...
/*it('adds a script with onload callback', function () {
    ...
    visit('http://localhost:8888')->assertSourceHas('data-onload="sa_event(\"My event\")"');
});*/
...
it('adds a script with overwrite domain name', function () {
    ...
    visit('http://localhost:8888')->assertSourceHas('data-hostname="example.com"');
});
...
it('adds a script with global variable name', function () {
    ...
    visit('http://localhost:8888')->assertSourceHas('data-sa-global="ba_event"');
});

it('adds automated events script when collect automated events is enabled', function () {
    asAdmin()->navigate('http://localhost:8888/wp-admin/options-general.php?page=simpleanalytics&tab=events')
        ->check('simpleanalytics_automated_events')
        ->click('Save Changes')
        ->assertChecked('simpleanalytics_automated_events');

    visit('http://localhost:8888')->assertPresent('script[src="https://scripts.simpleanalyticscdn.com/auto-events.js"]');
});
...
it('adds automated events script with auto collect downloads', function () {
    ...
    visit('http://localhost:8888')->assertSourceHas('data-collect="outbound,emails,downloads"');
});
...
it('adds automated events script with download file extensions', function () {
    ...
    visit('http://localhost:8888')->assertSourceHas('data-extensions="pdf,zip"');
});
...
it('adds automated events script with use titles of page enabled', function () {
    ...
    visit('http://localhost:8888')->assertSourceHas('data-use-title');
});
...
it('adds automated events script with use full urls enabled', function () {
    ...
    visit('http://localhost:8888')->assertSourceHas('data-full-urls');
});
...
it('adds a script with a custom domain name', function () {
    asAdmin()
        ->navigate('http://localhost:8888/wp-admin/options-general.php?page=simpleanalytics&tab=general')
        ->fill('simpleanalytics_custom_domain', 'mydomain.com')
        ->click('Save Changes')
        ->assertValue('simpleanalytics_custom_domain', 'mydomain.com');

    visit('http://localhost:8888')->assertPresent('script[src="https://mydomain.com/latest.js"]');
});
